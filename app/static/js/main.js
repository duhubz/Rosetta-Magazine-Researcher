...
/**
 * Segment-wise version comparison ("1.10" > "1.9", which parseFloat gets
 * wrong). Missing/non-numeric segments compare as 0. Returns >0, 0, or <0.
 */
function compareVersions(a, b) {
    const pa = String(a ?? '0').split('.').map(s => parseInt(s, 10) || 0);
    const pb = String(b ?? '0').split('.').map(s => parseInt(s, 10) || 0);
...
function renderLibrary() {
    ...
    catalogData.forEach(item => {
        ...
        if (isDownloaded) {
            const localMeta = metadataCache[localRelPath] || {};
            if (compareVersions(item.version, localMeta.version) > 0) {
...
function initToolbarDragAndDrop() {
    ...
    // Load saved layout (guard against corrupted localStorage JSON)
    let savedOrder = null;
    try {
        savedOrder = JSON.parse(localStorage.getItem('toolbarOrder'));
    } catch (err) {
        console.warn('Ignoring corrupted toolbarOrder in localStorage:', err);
        localStorage.removeItem('toolbarOrder');
    }
    if (savedOrder) {
...
**To safely close the application**, simply close your browser tab! The background server notices and shuts itself down about 20 seconds later. As long as the app is open in any tab — even one in the background — the server keeps running.
...
// Every open tab pings the server with a per-page-load tab id; the server
// stays alive while any tab remains registered — including hidden tabs,
// whose timers browsers throttle to roughly once a minute (still well
// inside the server's idle threshold). On pagehide the tab sends a
// "closing" beacon so the server can exit shortly after the last tab
// closes. After 3 consecutive ping failures we assume the server has shut
// down and show a banner asking the user to relaunch the app.
let pingFailureCount = 0;

// Fresh id per page load (deliberately NOT persisted: sessionStorage is
// copied into duplicated tabs, which would make two tabs share one id).
...
async function sendPing() {
    try {
        const res = await fetch(`/api/ping?tab=${TAB_ID}`);
...
// Goodbye on pagehide (sendBeacon works during unload; /api/ping accepts
// token-less POSTs for exactly this reason). The server waits a short
// grace period before exiting, so a refresh — goodbye followed by an
// immediate re-register from the reloaded page — survives.
window.addEventListener('pagehide', () => {
    if (navigator.sendBeacon) navigator.sendBeacon(`/api/ping?tab=${TAB_ID}&closing=1`);
});

// Fires on normal load AND on restore from the back/forward cache (which
// follows a pagehide goodbye) — re-register immediately in both cases.
