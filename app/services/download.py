...
def _download_worker_impl(task_id: str, item: dict[str, Any], mode: str = "full") -> None:
    ...
    try:
        ...
        if date_str:
            date_str = safe_name(date_str)
        issue_name = (
            meta.get("issue_name", item.get("issue_name", "")).replace("/", "_").replace("\\", "_")
        )
        if issue_name:
            issue_name = safe_name(issue_name)
    except ValueError:
    ...
    for k in [
        ...
        if item.get(k):
            ml.append(f"{k.replace('_', ' ').title()}: {item[k]}")
